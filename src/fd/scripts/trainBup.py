...
def train_k_folds(config: dict):
    ...
    for training_ids, testing_id in folds_ids_list:
        ...
        # Clear memory in preparation for next fold - address error: OOM when allocating tensor
        #gc.collect()
    ...
    #model_paths = [item for item in os.listdir('field-delineation/input-data/niva-cyl-models') if os.path.isdir(os.path.join('field-delineation/input-data/niva-cyl-models', item))]
    ...
    #weights = [model.load_weights(f'{model}/checkpoints/model.ckpt.data-00000-of-00001') for model in model_paths]
