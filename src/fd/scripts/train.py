...
def train_k_folds(config: dict):
    ...
    for training_ids, testing_id in folds_ids_list:
        ...
        model_path, callbacks = initialise_callbacks(ds_val, training_config, #SF added ds_val for visualisation prediction callback. Callback failed so removed. Too slow anyway
                                                    fold=left_out_fold)
